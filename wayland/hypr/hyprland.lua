...
-- Monitor profiles managed by hyprmon via ~/.config/hypr/monitors.conf
-- (Separate file so hyprmon can write to it without symlink issues)
-- Run `hyprmon` TUI to create/switch profiles:
--   Work: DP-3 (left), DP-4 (middle), eDP-1 (right)
--   Home: eDP-1 only

-- Source monitor profile config (generated by hyprmon or fallback below)
hl.source("~/.config/hypr/monitors.conf")

-- Fallback single-monitor config (auto-detect, only if monitors.conf is empty)
hl.monitor({
    output   = "",
    mode     = "preferred",
    position = "auto",
