...
function generateTheme(colorCodes, isDark = true) {
  ...
  // Pad to 11: foreground+cursor pickColor calls consume 2 more before selectDistinctColors(8)
...
function selectDistinctColors(colorObjects, count) {
  ...
  // Guarantee `count` entries — pad with analogous variants if input ran dry
  while (selectedColors.length < count) {
    selectedColors.push(
      selectedColors[Math.floor(Math.random() * selectedColors.length)].analogous()[3],
    );
  }
